...
(function () {
  ...
  /* ---------------------------------------------------------------------
     Install tabs (hero CLI install card)
     --------------------------------------------------------------------- */
  ...
  if (tabs.length) {
    tabs.forEach(t => {
      t.addEventListener('click', () => {
        ...
        cmds.forEach(c => {
          const isMatch = c.getAttribute('data-cmd') === target;
          c.hidden = !isMatch;
        });
      });
    });
  ...
  /* ---------------------------------------------------------------------
     Mini counter for hero float card
     --------------------------------------------------------------------- */
  const miniCtr = document.querySelector('[data-counter-mini]');
  if (miniCtr && 'IntersectionObserver' in window) {
    const target = parseInt(miniCtr.textContent, 10) || 0;
    miniCtr.textContent = '0';
    const ioMini = new IntersectionObserver((entries, obs) => {
      entries.forEach(e => {
        ...
        function tick(now) {
          ...
          const eased = 1 - Math.pow(1 - t, 3);
          miniCtr.textContent = Math.round(target * eased);
          if (t < 1) requestAnimationFrame(tick);
        }
        requestAnimationFrame(tick);
        obs.unobserve(e.target);
    ...
    ioMini.observe(miniCtr);
  }

  /* ---------------------------------------------------------------------
     Architecture KB counters (uses same animateCounter via reveal)
     --------------------------------------------------------------------- */
  const kbCounters = document.querySelectorAll('.arch__kbV .counter');
  if (kbCounters.length && 'IntersectionObserver' in window) {
    const ioKb = new IntersectionObserver((entries, obs) => {
      entries.forEach(e => {
        if (e.isIntersecting) {
    ...
    kbCounters.forEach(c => ioKb.observe(c));
  }
